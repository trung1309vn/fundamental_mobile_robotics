...
#---------------------------------------------------------------------
def compute_control_input(desired_state, robot_state, current_time, is_k_fixed, k_gain, max_vel, beta):
    ...
    if (not is_k_fixed):
        print(state_diff)
        k_gain = max_vel*(1-np.exp(-beta*np.abs(state_diff)))/(np.abs(state_diff)+0.0000001)
...
def simulate_control():
    ...
    for it in range(sim_iter):
        ...
        current_input = compute_control_input(desired_state, robot_state, current_time, False, None, np.array([2,2,1]), 2.5)
        #------------------------------------------------------------
